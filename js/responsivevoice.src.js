/**
 * @license ResponsiveVoice JS v1.5.0
 *
...
if (typeof responsiveVoice != 'undefined') {
    ...
    var ResponsiveVoice = function () {
        ...
        self.responsivevoices = [
            ...
            {name: 'US English Male', flag: 'us', gender: 'm', voiceIDs: [0, 4, 2, 6, 7, 75, 159, 234, 236, 237]},//[195,169]}, original service is (temporary?) down
        ...
        self.speak = function (text, voicename, parameters) {
            ...
            //iOS9 initialization
            if (self.iOS9 && !self.iOS9_initialized) {
                self.log("Initializing ios9");
                setTimeout(function(){ self.speak(text,voicename,parameters);},100);
                self.clickEvent();
                self.iOS9_initialized = true;
                return;
            }
            ...
            if (voicename == null) {
                self.setDefaultVoice('UK English Female');
                rv = self.default_rv;
            } else {
        ...
        self.getResponsiveVoice = function(name) {
            ...
            for (var i = 0; i < self.responsivevoices.length; i++) {
                if (self.responsivevoices[i].name == name) {
                    ...
                    //set fallbackMode and parts, used when the profile fallbackvoice is true or when fallbackMode is set manually
                    if (self.responsivevoices[i].mappedProfile.collectionvoice.fallbackvoice === true || self.fallbackMode === true) {
                        self.fallbackMode = true;
                        self.fallback_parts = [];
                    } else {
                        self.fallbackMode = false;
